...
対象は**乳幼児を育てる親**。情報は既にあるのに自分に該当する部分が分からない、という状態を、
**その人に必要な分だけに絞って**解消する。
...
| タブ | 中身 | 実装 |
|---|---|---|
| ランキング | よく行く場所を抽出し、災害リスクの高い順に並べる。各行に避難先まで出す | `src/ui/RankingTab.tsx` |
| 備蓄リスト | 子の**生まれ月の登録だけ**で、月齢に合わせた備蓄チェックリストを生成 | `src/ui/StocklistTab.tsx` |
| アンケート | 避難所の設備（授乳スペース・アレルギー対応・ベッドの有無）を現地で収集 | `src/ui/SurveyTab.tsx` |

ランキングの詳細シートには避難場所までの地図（`EvacuationMap.tsx`）と、
Gemini によるアドバイス欄（`GeminiComment.tsx`、鍵が無ければ非表示）が付く。

⚠️ **アンケートは現状「コンセプト実証」であって、機能としては完成していない。**
回答は `src/ui/storage.ts` 経由で**その端末の localStorage に保存されるだけ**で、
サーバーが無いため**他の利用者には共有されない**。
「集めた情報を、これから避難する人の行き先選びや物資の分配に活かす」という本来の狙いには、
回答を集約するバックエンドと、**データが集まるまでの期間をどう埋めるか**の設計が要る。
ここを未解決のまま提出した点が、この作品の一番の弱点。
...
```bash
npm test   # vitest。core/ のロジックのみ、58件
```
...
差し替えるなら `src/ui/EvacuationMap.tsx` と `src/ui/MapHome.tsx` の fetch 先
（`VITE_ORS_TOKEN` を読んでいるのはこの2ファイルだけ）を変えるだけで済むようにしてある。
...
アンケートタブは `navigator.geolocation` で**現在地を取得する**（今いる避難所を特定するため）。
取得した座標は端末内で割当区域の判定に使うだけで、外部へは送らない。
回答も localStorage に留まる（上の「機能」を参照）。
...
（5ファイル / 約6.3MB、最大ファイル4.65MB = `public/data/chochome.json`）。
ハッカソン特典が切れても運用は続く。

```bash
npm run deploy   # = npm run build && wrangler pages deploy dist --project-name cocoris
```
